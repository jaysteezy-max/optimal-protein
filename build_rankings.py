...
<style>
  ...
  *{box-sizing:border-box;margin:0}
  ...
  .sheet-scroll{overflow-y:auto; padding:6px 20px 30px; -webkit-overflow-scrolling:touch}
...
function openSheet(id){
  ...
  sheetBody.innerHTML = sheetHTML(i);
...
(function(){
  ...
  let y0 = null, dy = 0;
  handle.addEventListener('pointerdown', e => { y0 = e.clientY; dy = 0; sheet.style.transition = 'none'; handle.setPointerCapture(e.pointerId); });
  handle.addEventListener('pointermove', e => { if(y0==null) return; dy = Math.max(0, e.clientY - y0); sheet.style.transform = `translateY(${dy}px)`; });
  handle.addEventListener('pointerup', () => { if(y0==null) return; sheet.style.transition=''; sheet.style.transform=''; if(dy > 90) closeSheet(); y0=null; });
})();
